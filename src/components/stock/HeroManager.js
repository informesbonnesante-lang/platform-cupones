...
const HeroManager = () => {
  ...
  const handleUpdate = async (e) => {
    ...
    if (!error) {
      alert('ConfiguraciÃ³n de Hero actualizada con Ã©xito');
      fetchPageSettings();
    }
    setSaving(false);
  };

  if (loading) return <div style={{ textAlign: 'center', padding: '5rem' }}>Cargando configuraciÃ³n web...</div>;
    ...
    <div className="glass-card" style={{ padding: '2.5rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2.5rem' }}>
        <div>
          <h2 style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', margin: 0 }}>
            <Layout size={24} color="var(--primary)" /> GestiÃ³n de Contenidos Web
          </h2>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '2.5rem' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          <h3 style={{ fontSize: '1rem', color: 'var(--text-main)' }}>Seleccionar PÃ¡gina</h3>
          {pages.map(page => (
            <div 
              key={page.page_name}
            ...
            >
              ...
              <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', margin: 0 }}>Ãšltima ediciÃ³n tÃ©cnica vÃ­a API</p>
            </div>
          ))}
        </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
              <div>
                <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem', fontWeight: 600 }}>
                  <Type size={16} /> TÃ­tulo Principal (Hero Title)
                </label>
                <input 
                  type="text" 
              ...
              <div>
                <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem', fontWeight: 600 }}>
                  <Type size={16} /> SubtÃ­tulo (Hero Subtitle)
                </label>
                <textarea 
                  className="input-field"
              ...
              <div style={{ padding: '1.5rem', background: 'white', borderRadius: '12px', border: '1px solid var(--border)' }}>
                <p style={{ fontSize: '0.85rem', fontWeight: 700, marginBottom: '0.5rem' }}>PrevisualizaciÃ³n de Estilo</p>
                <div style={{ 
                  height: '100px', 
                  borderRadius: '8px', 
